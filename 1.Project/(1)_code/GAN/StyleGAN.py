...
BATCH_SIZE = 4          
directory = "Gan/FFHQ"  # image 폴더
n_images = 70000         # data 수
suff = 'png'            # image 확장자명
...
from gan07_AdaIN import AdaInstanceNormalization
...
class WGAN(object):
    ...
    def __init__(self, steps = -1, lr = 0.0001, silent = True):
        ...
        self.im = dataGenerator(directory, n_images, suffix = suff, flip = True)    # 어디서 어떻게 가져올지 정함
    ...
    def evaluate(self, num = 0, trunc = 2.0): #8x4 images, bottom row is constant
        ...
        x.save("D:/Study/project/GAN/result/stylegan/i"+str(num)+".jpg")
    ...
    def evalTrunc(self, num = 0, trunc = 1.8):
        ...
        x.save("D:/Study/project/GAN/result/stylegan/t"+str(num)+".jpg")         # image 저장 -> 경로
    ...
    def saveModel(self, model, name, num): #Save a Model
        ...
        ''' 
        model.save_weights("D:/Study/project/GAN/models/"+name+"_"+str(num)+".h5")
        
    def loadModel(self, name, num): #Load a Model
        
        file = open("D:/Study/project/GAN/models/"+name+".json", 'r')
        json = file.read()
        file.close()
        
        mod = model_from_json(json, custom_objects = {'AdaInstanceNormalization': AdaInstanceNormalization})
        mod.load_weights("D:/Study/project/GAN/models/"+name+"_"+str(num)+".h5")
